				...
				<form:form action="${action}" method="post" commandName="specialization">
					...
					<div class="btn-group pull-right">
						<c:if test="${(alreadyExists) && (not (state == 'run'))}">
							<c:choose>
								<c:when test="${specialization.type == specialization.JOB_SECTOR}">
									<a name="delete" class="btn btn-danger" href="${pageContext.request.contextPath}/admin/config/delete/job-sector/${abbreviation}">Supprimer</a>
								</c:when>
								<c:otherwise>
									<a name="delete" class="btn btn-danger" href="${pageContext.request.contextPath}/admin/config/delete/improvement-course/${abbreviation}">Supprimer</a>
								</c:otherwise>
							</c:choose>
						</c:if>
